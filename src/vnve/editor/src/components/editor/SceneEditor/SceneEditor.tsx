...
export function SceneEditor() {
  ...
  const progressAnimationId = useRef<ReturnType<typeof setTimeout> | 0>(0);
  ...
  useEffect(() => {
    ...
    if (scriptText) {
      localStorage.removeItem('vnve_script_text');
      handleOpenImportText2Scene(scriptText);
    }
  }, []);
  ...
  const updateActionProgress = (progress, currentTime) => {
    if (progress >= 100) {
      ...
      if (progressAnimationId.current) {
        clearTimeout(progressAnimationId.current as any);
      }
