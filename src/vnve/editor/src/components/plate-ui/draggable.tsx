...
import { type DragItemNode, useDraggable } from "@udecode/plate-dnd";
...
export interface DraggableProps extends PlateElementProps {
  classNames?: Partial<ClassNamesProp>;
  /**
...
export const Draggable = withRef<"div", DraggableProps>(
  ({ className, classNames = {}, onDropHandler, ...props }, ref) => {
    ...
    const state: any = { element, onDropHandler };
    const { dropLine, isDragging, isHovered } = state as any;
    const draggable: any = useDraggable(state as any);
    const {
      droplineProps,
      groupProps,
      gutterLeftProps,
