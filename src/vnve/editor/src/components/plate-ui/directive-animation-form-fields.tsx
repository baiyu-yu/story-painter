...
import { DirectiveName, Sprite, isChild } from "@vnve/core";
  ...
  useEffect(() => {
    if (activeScene) {
      ...
      for (const child of activeScene.children) {
        if (!isChild(child)) continue;
        const newOption = {
          value: child.name,
          label: child.label,
          assetID: (child as Sprite).assetID,
        };
        const sprite = child as Sprite;
        if (sprite.assetType === DBAssetType.Character) {
          newCharacterOptions.push(newOption);
        } else if (sprite.assetType === DBAssetType.Background) {
          newBackgroundOptions.push(newOption);
        } else if (sprite.assetType === DBAssetType.Thing) {
  ...
  useEffect(() => {
    if (
      ...
      const targetChild = activeScene.children.find(
        (item): item is Sprite => isChild(item) && (item as Sprite).name === formTargetName,
      ) as Sprite;
