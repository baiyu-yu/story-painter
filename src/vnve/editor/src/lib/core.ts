  ...
  for (let i = 0; i < story.length; i++) {
    ...
    for (const dialogue of item.dialogues) {
      ...
      if (dialogue.character.name === "旁白") {
        ...
        if (asset) {
          const speakerSprite = scene.findChildByLabel(name) as Sprite;
      ...
      let lineText = dialogue.line;
      let transitionDirective;

      // Parse Transition: [Transition:EffectName]
      const transitionMatch = lineText.match(/\[Transition:(.+?)\]/);
      if (transitionMatch) {
        const transitionName = transitionMatch[1];
        lineText = lineText.replace(transitionMatch[0], "");
        
        transitionDirective = {
          type: "directive",
          value: {
            directive: transitionName.endsWith("Transition") ? transitionName : transitionName + "Transition",
            params: {
              duration: 1,
            },
            label: `转场:${transitionName}`,
          },
          children: [
            {
              text: "",
