...
import { Child, reviveChildren, Sprite, isChild } from "./child";
...
export class Scene extends PIXI.Container {
  ...
  public sounds: Sound[];
  public declare children: PIXI.Container[];
  ...
  public findChildByLabel(label: string) {
    return (this.children as unknown as Child[]).find(
      (item) => (item as any).label === label,
    );
  }

  public getChildrenByAssetType(assetType: string) {
    return (this.children as unknown as Child[]).filter(
  ...
  public clone(): Scene {
    ...
    const children = (this.children as unknown as Child[]).map((item) =>
      (item as Child).clone(true),
    );
