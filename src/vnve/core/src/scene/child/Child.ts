...
export function isChild(obj: PIXI.Container): obj is Child {
  return (
    obj instanceof Sprite ||
    obj instanceof Text ||
    obj instanceof Graphics ||
    obj instanceof AnimatedGIF ||
...
export function copyTo(
  ...
  to.skew.x = from.skew.x;
  to.skew.y = from.skew.y;
  to.pivot.x = from.pivot.x;
  to.pivot.y = from.pivot.y;
  if (!ignoreWH) {
    to.width = from.width;
...
export async function copyFromJSON(from: AnyJSON, to: Child, ignoreWH = false) {
  ...
  to.alpha = from.alpha;
  to.zIndex = from.zIndex;
  const [x, y, sx, sy, rotation, kx, ky, px, py] = from.transform;
  to.x = x;
  to.y = y;
  to.scale.x = sx;
  to.scale.y = sy;
...
function getTransformArray(child: PIXI.Container) {
  return [
    child.x,
    child.y,
    child.scale.x,
