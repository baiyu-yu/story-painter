...
export class Editor {
  ...
  public setActiveChild(child?: Child) {
    ...
    if (child) {
      ...
      if (this.activeTransformer) {
        (this.activeTransformer as any).group = [child];
  ...
  public setActiveChildByName(name: string) {
    ...
    if (scene) {
      const child = scene.children.find(
        (item): item is Child => isChild(item) && item.name === name,
      );
  ...
  private genSceneScript(scene: Scene): SceneScript {
    ...
    const sceneBackground = scene.children.find(
      (item): item is Sprite => item instanceof Sprite && item.assetType === "Background",
    );
