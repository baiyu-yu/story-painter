...
export class Speaker extends AnimationDirective<PIXI.Text> {
  ...
  public execute() {
    ...
    if (this.target) {
      ...
      if (isDice) {
           const diceText = new PIXI.Text("Dice", { fontSize: 60 });
           diceText.anchor.set(0.5);
           // Position relative to target (nameplate)
          diceText.x = this.target.x + this.target.width / 2 + 50; 
          diceText.y = this.target.y;
          diceText.zIndex = 999;
          this.stage.addChild(diceText);
          
          // Simple animation
          import("gsap").then(({ default: gsap }) => {
            gsap.from(diceText.scale, { x: 0, y: 0, duration: 0.5, ease: "back.out" });
            gsap.to(diceText, { alpha: 0, duration: 0.5, delay: 2, onComplete: () => diceText.destroy() });
          });
      }
    }
